...
public protocol CustomActiveLabelDelegate: AnyObject {
    func didSelect(_ text: String, type: CustomActiveType)
}

public typealias ConfigureLinkAttribute = (CustomActiveType, [NSAttributedString.Key : Any], Bool) -> ([NSAttributedString.Key : Any])
typealias ElementTuple = (range: NSRange, element: CustomActiveElement, type: CustomActiveType)

@IBDesignable open class CustomActiveLabel: UILabel {
    
    // MARK: - public properties
    open weak var delegate: CustomActiveLabelDelegate?
    
    open var enabledTypes: [CustomActiveType] = [.mention, .hashtag, .url]
    ...
    open var customColor: [CustomActiveType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    open var customSelectedColor: [CustomActiveType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    open func handleCustomTap(for type: CustomActiveType, handler: @escaping (String) -> ()) {
        customTapHandlers[type] = handler
    }
    
    open func removeHandle(for type: CustomActiveType) {
        switch type {
        case .hashtag:
    ...
    // MARK: - customzation
    @discardableResult
    open func customize(_ block: (_ label: CustomActiveLabel) -> ()) -> CustomActiveLabel {
        _customizing = true
        block(self)
        _customizing = false
        updateTextStorage()
    ...
    internal var customTapHandlers: [CustomActiveType : ((String) -> ())] = [:]
    ...
    lazy var activeElements = [CustomActiveType: [ElementTuple]]()
    ...
    //MARK: - CustomActiveLabel handler
    ...
    fileprivate func didTap(_ element: String, for type: CustomActiveType) {
...
extension CustomActiveLabel: UIGestureRecognizerDelegate {
